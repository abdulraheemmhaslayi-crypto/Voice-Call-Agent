...
export default function WorkflowRunPage() {
    ...
    useEffect(() => {
        const fetchWorkflowRun = async () => {
            ...
            // Prevent the UI from being stuck forever when backend is slow/unavailable.
            // After timeout, we show a retry UI instead of loading skeleton for ~10+ minutes.
            const TIMEOUT_MS = 25000;

            const timeoutPromise = new Promise<never>((_, reject) => {
                setTimeout(() => reject(new Error(`Run details fetch timed out after ${TIMEOUT_MS}ms`)), TIMEOUT_MS);
            });
        ...
        fetchWorkflowRun().catch((e: unknown) => {
            setIsLoading(false);
            const msg = e instanceof Error ? e.message : 'Failed to fetch run details';
            setLoadingError(msg);
        });
                                ...
                                {workflowRun?.gathered_context?.trace_url && (
                                    <div className={`flex items-center gap-2 ${isTextChatRun ? '' : 'border-l border-border pl-4'}`}>
                                        ...
                                        <Button
                                            ...
                                            className="gap-2"
                                        >
                                            <a href={String(workflowRun.gathered_context.trace_url)} 
                                            target="_blank"
